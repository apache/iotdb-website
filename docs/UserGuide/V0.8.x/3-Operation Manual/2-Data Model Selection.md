...
### Show Storage Group
After the storage group is created, we can use the [SHOW STORAGE GROUP](../5-IoTDB%20SQL%20Documentation/1-IoTDB%20Query%20Statement.html) statement to view all the storage groups. The SQL statement is as follows:
...
Please refer to [Encoding](../2-Concept%20Key%20Concepts%20and%20Terminology/3-Encoding.html) for correspondence between data type and encoding.
...
Limit 1: Assuming that the JVM memory allocated to IoTDB at runtime is p and the user-defined size of data in memory written to disk ([group\_size\_in\_byte](../4-Deployment%20and%20Management/2-Configuration.html)) is Q, then the number of storage groups should not exceed p/q.
