...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>该消息将订阅 IoTDB 的所有数据，不包括乱序数据，按照一定时间进行过滤，同时只需要大于100的数据。</p><p>此外，还需要自定义 socket.onmessage 函数，以处理获取到的数据。此外，还需要编写 socket.onclose 和 socket.onerror 等函数，以自定义客户端对这些事件的响应。</p><h2 id="mqtt-方式订阅" tabindex="-1"><a class="header-anchor" href="#mqtt-方式订阅" aria-hidden="true">#</a> MQTT 方式订阅</h2><p>目前 IoTDB 的 MQTT 方式支持数据订阅功能，其数据格式与 WebSocket 相同。mqtt 的 host 和 port 需要在 iotdb 的 properties 文件内配置。</p><h2 id="数据过滤" tabindex="-1"><a class="header-anchor" href="#数据过滤" aria-hidden="true">#</a> 数据过滤</h2><p>与数据同步软件相同，IoTDB 的订阅功能也可以提供数据过滤的功能。WebSocket 与 MQTT 方式已经给出了示例。<br> Api 方式下，以 Java 订阅接口为例子，用户可在 SubscriptionConfiguration 中配置过滤条件（Strategy）。目前支持指定的条件包括：</p><ul><li>是否需要过滤乱序数据（disorderHandlingStrategy）</li><li>需要订阅的序列的共同前缀（topicsStrategy）</li><li>指定序列的时间范围（timeStrategy）</li><li>指定序列的值范围（valueStrategy）</li></ul><div class="language-aidl line-numbers-mode" data-ext="aidl"><pre class="language-aidl"><code>public class PushConsumerExample {
...
`),p])}const r=n(i,[["render",o],["__file","IoTDB-Data-Subscription_timecho.html.vue"]]);export{r as default};
